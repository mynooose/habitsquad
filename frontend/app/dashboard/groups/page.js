    ...
    <div className="p-8 max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex gap-3">
          <button onClick={() => setShowJoin(true)} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-surface-100 hover:bg-surface-200 text-sm font-medium">
            ...
            <form onSubmit={handleJoin} className="space-y-4">
              <div>
                ...
                <input type="text" value={inviteCode} onChange={e => setInviteCode(e.target.value)} placeholder="Enter invite code"
                  className="w-full px-4 py-3 rounded-xl bg-surface-200 border border-white/10 placeholder-zinc-500 focus:outline-none focus:border-brand-500" />
              </div>
              <div className="flex gap-3">
                <button type="button" onClick={() => setShowJoin(false)} className="flex-1 py-3 rounded-xl bg-surface-200 font-medium hover:bg-surface-300">Cancel</button>
      ...
      {groups.length === 0 ? (
        <div className="text-center py-16 rounded-2xl bg-surface-100 border border-white/5">
          ...
          <div className="flex justify-center gap-3">
            <button onClick={() => setShowJoin(true)} className="px-6 py-3 rounded-xl bg-surface-200 font-medium hover:bg-surface-300">Join Group</button>
        ...
        <div className="grid gap-4">
          {groups.map((group, i) => (
            <Link key={group.id} href={`/dashboard/groups/${group.id}`}
              className="p-5 rounded-2xl bg-surface-100 border border-white/5 flex items-center gap-4 hover:border-white/10 transition-all animate-slide-up"
